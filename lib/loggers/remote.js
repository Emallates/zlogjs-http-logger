    ...
    var pre_write = res.write,
     ...
     res.on('finish', function(){
       ...
       function logRequest(){
        ...
        var skip = MANUAL === true ? false : creds.configs.SKIP;
        var skipCode = MANUAL === true ? false : creds.configs.SKIP_CODE;
        if(skipCode){
        ...
        _prelog({req:req, res:res, base: base}, creds, opts);
      }
    });
    next();
  }
...
function _prelog(obj, creds, opts){
  if(obj.req.method != "GET")
    obj.req.on('data', function(data){
      _process(obj, creds, opts, data || "");
    });
  else _process(obj, creds, opts, "");
}

function _process(obj, creds, options, data){
  ...
  var base = obj.base;
     ...
     creds.context._sendRequest({body:mbody, url:'http://'+creds.configs.host+':'+creds.configs.port+base.input, timeout:creds.configs.timeout, method:"POST"}, function(err, data){  
